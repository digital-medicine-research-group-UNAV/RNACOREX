...
def preprocessing_rnacorex(bbdd):
    ...
    gtf_path = os.path.join('../StructuralEngine/gencode.v47.basic.annotation.gtf')
    ...
    deg_mrna = deg(filtered_data["mrna"], classvalues, '0', '1', 0.05)

    print(filtered_data["mrna"].shape)
    print(deg_mrna.shape)

    deg_data = pd.concat([deg_mrna, filtered_data["mirna"]], axis = 1)

    num_zeros = (deg_data == 0).sum().sum()

    print(f'Number of zeros in the dataset: {num_zeros}')

    normalized_data = deg_data.apply(zscore)

    data_def = pd.concat([normalized_data, classvalues], axis=1)

    data_def.columns = data_def.columns.str.split('.').str[0]

    data_def.to_csv('Data/data_plos_'+bbdd+'.csv')