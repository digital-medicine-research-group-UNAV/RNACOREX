...
for notebook in tqdm(notebook_names):
    ...
    accuracy_results.to_csv('PLOS Results/results_'+notebook+'.csv', index=False)
    ...
    mrnc.fit_only()
    # mrnc.connections_.to_csv('PLOS Results/connections_'+notebook+'.csv', index=False)
    mrnc.get_network(k=accuracy_results['Test'].idxmax(), save = 'PLOS Results/network_'+notebook+'.svg')
