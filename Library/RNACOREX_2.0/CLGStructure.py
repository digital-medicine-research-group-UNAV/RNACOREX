...
def pl_plemclg(dataset, classpl, dag, filtered_fmi, filtered_smi, info):
    ...
    rl = {"classp":classp,"beta":beta,"sigma":sigma,"dag":dag,"labels":info["labels"],"types":info["types"],"names":info["names"],"fmi":filtered_fmi,"smi":filtered_smi,"ll":ll,"acc":accuracy,"bic":bic}
...
def get_interactions(clgc, n):
    ...
    print(smi.shape)
    print('NONZERO:', np.count_nonzero(smi))

    for i in range(int(n_it)):

        max_smi_index = np.unravel_index(np.argmax(smi), smi.shape)
        element1 = clgc["names"][max_smi_index[0]]
        element2 = clgc["names"][max_smi_index[1]]
        type1 = clgc["types"][max_smi_index[0]]
        type2 = clgc["types"][max_smi_index[1]]

        interactions.append({
            "Node1": element1,
            "Node2": element2,
            "SMI": smi[max_smi_index],
            "FMI": fmi[max_smi_index],
            "Type1": type1,
            "Type2": type2
        })
        ...
        if i*2 <= n:
            ...
            max_fmi_index = np.unravel_index(np.argmax(fmi), fmi.shape)
            element1 = clgc["names"][max_fmi_index[0]]
            element2 = clgc["names"][max_fmi_index[1]]
            type1 = clgc["types"][max_fmi_index[0]]
            type2 = clgc["types"][max_fmi_index[1]]

            interactions.append({
                "Node1": element1,
                "Node2": element2,
                "SMI": smi[max_fmi_index],
                "FMI": fmi[max_fmi_index],
                "Type1": type1,
                "Type2": type2
            })
