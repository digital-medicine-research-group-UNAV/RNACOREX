...
def get_interactions(smi, fmi, info, n):
    ...
    for i in tqdm(range(int(n_it))):

        max_smi_index = np.unravel_index(np.argmax(smi), smi.shape)
        element1 = info["names"][max_smi_index[0]]
        element2 = info["names"][max_smi_index[1]]
        type1 = info["types"][max_smi_index[0]]
        type2 = info["types"][max_smi_index[1]]
        ...
        if i*2 <= n:
            ...
            element1 = info["names"][max_fmi_index[0]]
            element2 = info["names"][max_fmi_index[1]]
            type1 = info["types"][max_fmi_index[0]]
            type2 = info["types"][max_fmi_index[1]]
