...
def rename_all(gene_list, gtf):
    extra = pd.read_csv('Otros/conversor2.txt', sep='\s+', header=None)
...
def ens_to_gen(ens, gtf):
    ...
    filtered_rows = df_extra[df_extra['gene_id'] == ens]
    gen = filtered_rows['gene_name'].values[0]
